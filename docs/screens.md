...
`PauseMenuPopup` extends `PopupWidget` and provides an ESC-triggered pause overlay during gameplay: a centered menu listing Resume / Settings / Exit to Main Menu (selected via Up/Down arrow keys, confirmed via Enter; Escape dismisses or selects Resume). It follows the same `PopupWidget` pattern as Inventory/Codex — added to the game's `JLayeredPane` at `POPUP_LAYER` in `Main.buildGameCard` and toggled open/closed via a `PauseToggleListener` (implements `GameListener`) registered with `GamePanel`, same as `PopupToggleListener` handles Inventory/Codex mutual exclusion and focus restoration. When opened, the pause menu sets `GamePanel.paused = true` to freeze movement input (the `MoveAction` inner class checks this flag before moving the player), ensuring the game actually pauses rather than merely hiding the view behind the popup. Selecting Resume or pressing Escape again dismisses and resumes gameplay (`setOnDismiss` callback resets `paused = false`). Selecting Settings navigates to the Settings screen with `backTarget = "pause"` so that backing out returns to the pause menu rather than the title screen (game remains paused underneath). Selecting Exit to Main Menu calls `Main.resetGame()` to dispose the frame and rebuild everything from scratch, landing on the title card — the same mechanism F5 hot-reset uses.
